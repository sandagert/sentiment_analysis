...
from config import (
    PROMPTS, BERT_CONFIG, ROBERTA_CONFIG,
...
if args.model in ['bert', 'roberta']:
    ...
    # IMPORTANT: use model context & left truncation so the final review/question is preserved
    max_ctx = getattr(model.config, "max_position_embeddings", 4096)
    tokenizer.model_max_length = max_ctx
    tokenizer.truncation_side = "left"

    print(f"  ✓ Loaded from {model_dir} (context length: {max_ctx})")
...
def _build_prompt_with_examples(tokenizer, base_prompt: str, examples: list, review: str) -> str:
    ...
    if examples:
        ...
        for i, (review_text, label) in enumerate(examples, 1):
            review_short = review_text[:300] + "..." if len(review_text) > 300 else review_text
            label_text = "positive" if label == 1 else "negative"
            examples_text += f"{i}. Review: {review_short}\n   Label: {label_text}\n\n"

    # Use the base_prompt template (same as zero-shot)
    if "{review}" in base_prompt:
        formatted_prompt = base_prompt.format(review=review)
    else:
        formatted_prompt = base_prompt + "\n\nReview:\n" + review

    user_text = (
        f"{examples_text}"
        f"Now classify this review using the same approach:\n\n"
        f"{formatted_prompt}\n\n"
...
def evaluate_llama(model, tokenizer, prompt, reviews, few_shot_examples=None):
    ...
    for idx, review in enumerate(reviews):
        ...
        # Diagnostic for the first prompt
        if idx == 0:
            full_len = len(tokenizer(prompt_text, add_special_tokens=False, truncation=False)["input_ids"])
            max_len = getattr(tokenizer, "model_max_length", 4096)
            print(f"\n  📊 First prompt stats:")
            print(f"     Tokens: {full_len}")
            print(f"     Few-shot: {few_shot_examples is not None}")
            if full_len > max_len - 8:
                print(f"     ⚠️ Will be truncated to {max_len - 8} tokens")
            print()
...
prompt_counter = 0
expected_total_prompts = sum(len(v) for v in PROMPTS.values())
...
for category, prompts in PROMPTS.items():
    ...
    for i, prompt in enumerate(prompts):
        prompt_counter += 1
        print(f"\n[{prompt_counter}/{expected_total_prompts}] Variation {i + 1}/{len(prompts)}")
...
total_prompts = prompt_counter
if total_prompts != expected_total_prompts:
    print(f"⚠️ Warning: expected {expected_total_prompts} prompts, actually ran {total_prompts}")

true_labels_repeated = true_labels * total_prompts
assert len(true_labels_repeated) == len(all_predictions), \
    f"Label length {len(true_labels_repeated)} != predictions length {len(all_predictions)}"
...
fig_dir = EVAL_CONFIG['results_dir'].replace('results', 'figures')
...
if args.model in ['llama_7b', 'llama_13b'] and FEW_SHOT_CONFIG['enabled']:
    cm_file = f"{fig_dir}/{args.model}_{FEW_SHOT_CONFIG['n_shots']}shot_confusion_matrix.png"
else:
    cm_file = f"{fig_dir}/{args.model}_confusion_matrix.png"
