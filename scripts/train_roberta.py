# ============================================================================
# Train RoBERTa on IMDB sentiment analysis
# ============================================================================

import torch
from datasets import load_from_disk
from transformers import AutoTokenizer, AutoModelForSequenceClassification, Trainer, TrainingArguments
from config import ROBERTA_CONFIG, TEST_MODE, TEST_CONFIG
...
print("\n2. Loading RoBERTa model...")

# Load pre-trained RoBERTa with classification head
model = AutoModelForSequenceClassification.from_pretrained(
    ROBERTA_CONFIG["model_name"],
    num_labels=2
)

print(f"   ✓ Loaded {ROBERTA_CONFIG['model_name']}")
...
num_epochs = TEST_CONFIG["num_epochs"] if TEST_MODE else ROBERTA_CONFIG["num_train_epochs"]

training_args = TrainingArguments(
    output_dir=ROBERTA_CONFIG["output_dir"],
    num_train_epochs=num_epochs,
    per_device_train_batch_size=ROBERTA_CONFIG["per_device_train_batch_size"],
    per_device_eval_batch_size=ROBERTA_CONFIG["per_device_eval_batch_size"],
    gradient_accumulation_steps=ROBERTA_CONFIG["gradient_accumulation_steps"],
...
trainer.save_model(ROBERTA_CONFIG["final_model_dir"])

# Also save tokenizer in same location
tokenizer = AutoTokenizer.from_pretrained(ROBERTA_CONFIG["model_name"])
tokenizer.save_pretrained(ROBERTA_CONFIG["final_model_dir"])

print(f"   ✓ Model saved to {ROBERTA_CONFIG['final_model_dir']}")
...
print("="*80)
print(f"\nFinal model saved to: {ROBERTA_CONFIG['final_model_dir']}")
print("You can now run: python evaluate.py --model roberta")
print("="*80)